...
Naomi has mainly two ways to be accessed:
...
Naomi has a built-in webserver, which listens on port 8080 for HTTP and 8443 for HTTPS requests.
In general, it is advised to use HTTPS communication over HTTP.

The default ports 8080 and 8443 can be changed by setting the environment variables `NAOMI_HTTP_PORT` resp. `NAOMI_HTTPS_PORT`.
In an apt installation, you would best do this in the file `/etc/default/naomi`.

### SSL Certificates

On the very first start, Naomi generates a personal (self-signed, 256-bit ECC) SSL certificate and stores it in the Jetty keystore (in `${USER_DATA}etc/keystore`).
...
Naomi does not (yet) support restricting access through HTTP(S) for certain users - there is no authentication in place, nor is there a limitation of functionality or information that different users can access.

**Security Warning:**
It is vitally important that you MUST NOT directly expose your Naomi instance to the Internet (e.g. by opening a port in your firewall)!

If you want to limit access to only certain network interfaces, you can do so by setting the environment variable `NAOMI_HTTP_ADDRESS`.
Setting it to

```conf
NAOMI_HTTP_ADDRESS=127.0.0.1
```
...
Clearly, having remote access to your Naomi instance is something most users would not want to miss.
There are different options to do so.

### VPN Connection

The most secure option is probably to create a VPN connection to your home network.
Doing so will allow you to access your Naomi instance in the same way as if you were at home.
There are many different solutions for VPN, so we cannot give any specific advice here, what to use and how to set in up.

### myNaomi Cloud Service

You can use an [Naomi Cloud](https://github.com/naomiproject/naomi-cloud/blob/master/README.md) instance to which Naomi  creates a tunnel connection and which forwards all requests through this tunnel.
Naomi will see these incoming requests as originating from the local loopback interface.

The simplest way to get hold of such an Naomi Cloud is to register an account at [myNaomi.projectnaomi.com](http://myNaomi.projectnaomi.com/), which is operated by the [Core Project Naomi Team](https://projectnaomi.com/).

### Running Naomi Behind a Reverse Proxy

A reverse proxy simply directs client requests to the appropriate server.
This means you can proxy connections to `http://mydomain_or_myip` to your Naomi runtime.
You just have to **replace *mydomain_or_myip*** with either an **internal or external IP** (e.g. xx.xx.xx.xx) or a **domain** if you own one that links to the external IP of Naomi (e.g. naomi.mydomain.tld).

Running Naomi behind a reverse proxy allows you to access your Naomi runtime via port 80 (HTTP) and 443 (HTTPS).
It also provides you a simple way of **protecting your server** with authentication and secure certificates.

The good news is that [Naobian](naobian) already offers the possibility to activate a preconfigured NGINX reverse proxy, which includes setting up authentication and a valid [Let's Encrypt](https://letsencrypt.org) certificate.
...
The location of the default setup is `/etc/nginx/sites-enabled/default`. To allow NGINX to proxy Naomi, you need to change this file (make a backup of it in a different folder first).

The recommended configuration below assumes that you run the reverse proxy on the same machine as your Naomi runtime.
If this doesn't fit for you, you just have to replace `proxy_pass http://localhost:8080/` by your Naomi runtime hostname (such as `http://yournaomihostname:8080/`).
...
```shell
sudo mv /etc/nginx/sites-enabled/default /etc/nginx/sites-enabled/naomi
```
...
...and then go to `http://mydomain_or_myip` to see your Naomi server.

#### Authentication with NGINX

For further security, you may wish to ask for a **username and password** before users have access to Naomi.
...
Now the configuration file (`/etc/nginx/sites-enabled/naomi`) needs to be edited to use this password.
...
It is often desirable to allow specific IPs (e.g. the local network) to access Naomi without needing to prompt for a password or to block everyone else entirely.
...
If you need to use an internal or external IP to connect to Naomi, follow the [instructions for OpenSSL](#nginx-openssl)
...
```shell
sudo openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /etc/ssl/naomi.key -out /etc/ssl/naomi.crt
```
...
```nginx
  ssl_certificate                 /etc/ssl/naomi.crt;
  ssl_certificate_key             /etc/ssl/naomi.key;
```
...
Since the proxy only forwards traffic to the Naomi server, the server needs to be told to handle requests at this address differently.
...
##### Adding the Certificates to the Proxy Server
...
After following all the steps on this page, you *should* have a NGINX server configuration (`/etc/nginx/sites-enabled/naomi`) that looks like this:
...
server {
    ...
    ssl_certificate                 /etc/letsencrypt/live/mydomain/fullchain.pem; # or /etc/ssl/naomi.crt
    ssl_certificate_key             /etc/letsencrypt/live/mydomain/privkey.pem;   # or /etc/ssl/naomi.key
...
This optional section is for those who would like to strengthen the HTTPS security on Naomi, it can be applied regardless of which HTTPS method you used [above](#nginx-https), **but you need to follow at least one of them first**.
...
If you're achieving A or A+ here, then your client-naomi communication is very secure.
